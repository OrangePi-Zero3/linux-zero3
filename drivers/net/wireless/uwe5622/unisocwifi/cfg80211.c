					...
					u8 status_code)
{
	...
	else if (vif->sm_state == SPRDWL_CONNECTED && conn_info->status == SPRDWL_ROAM_SUCCESS) {
...
static void sprdwl_cfg80211_update_mgmt_frame_registrations(struct wiphy *wiphy,
							    struct wireless_dev *wdev,
							    struct mgmt_frame_regs *upd)
{
	...
	if (upd->interface_stypes & BIT(mgmt_type) == 0) {
		return;
	}

	wl_ndev_log(L_DBG, wdev->netdev, "frame_type %d, reg %d\n", upd->interface_stypes, upd->interface_stypes & BIT(IEEE80211_STYPE_PROBE_REQ >> 4));
	...
	reg_mgmt->type = upd->interface_stypes;
...
static struct cfg80211_ops sprdwl_cfg80211_ops = {
	...
	.update_mgmt_frame_registrations = sprdwl_cfg80211_update_mgmt_frame_registrations,
